...
def test_legacy_monkeypatch_precedes_fresh_canonical_consumer_import(monkeypatch):
    ...
    A legacy name is an alias, so dropping it alone leaves the canonical module
    loaded and cached. Re-execution is what makes the seam patch visible, so the
    canonical name is dropped too; both names still resolve to one object.
    """
    logger.debug("test_legacy_monkeypatch_before_fresh_consumer entry")
    module_name = "src.core.finite_builder_runtime"
    canonical_name = "src.core.construction.finite_builder.runtime"
    package = importlib.import_module("src.core")
    originals = {
        name: sys.modules.get(name, _MISSING) for name in (module_name, canonical_name)
    }
    ...
    with monkeypatch.context() as patch:
        ...
        try:
            for name in (module_name, canonical_name):
            ...
            assert fresh is importlib.import_module(canonical_name)
        finally:
