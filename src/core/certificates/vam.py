...
from ..registry.theorem_language import TheoremEnvironment
from vam.src import (
    canonical_report,
    classify_program,
...
from .vam_optimizer import certify_vam_optimizer_gate
...
def certify_vam_reference_v1() -> Certificate:
    ...
    native_dense_cli = native_cli and (root / "tests/vam/test_vam_native_vamd_executor.py").exists()
    ...
    native_optimizer_slice = native_cli and all(path.exists() for path in native_optimizer_paths) and (root / "tests/vam/test_vam_native_optimizer.py").exists()
    native_optimizer_extension = native_optimizer_slice and (root / "tests/vam/test_vam_native_optimizer_expansion.py").exists()
    native_vamd_optimizer_policy = native_optimizer_extension and (root / "tests/vam/test_vam_native_optimizer_generated.py").exists() and (root / "vam/docs/024_vam_v1_6_vamd_optimizer_and_generated_parity.md").exists()
    native_vam0_emission = native_vamd_optimizer_policy and (root / "tests/vam/test_vam_native_emit_optimized_vam0.py").exists() and (root / "vam/docs/025_vam_v1_7_optimized_vam0_emission.md").exists()
    optimizer_witness_gate = (
        (root / "vam/src/optimizer_witness.py").exists()
        and (root / "vam/src/optimizer_obligations.py").exists()
        and (root / "tests/vam/test_vam_optimizer_witness.py").exists()
        and (root / "tests/vam/test_vam_optimizer_obligations.py").exists()
    ...
    native_optimizer_metamorphic = native_vam0_emission and (root / "tests/vam/test_vam_native_optimizer_metamorphic.py").exists()
    native_vamd_boundaries = (root / "tests/vam/test_vam_native_vamd_boundaries.py").exists()
    native_parity_expanded = (root / "tests/vam/test_vam_native_parity_expansion.py").exists()
