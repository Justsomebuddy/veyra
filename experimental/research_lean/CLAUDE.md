...
- `manifest.json` is canonical: exactly 48 stable dependencies, nine research
...
- Added the exact-replay-passed singleton-tact path-word bridge as a ninth
  manifest-governed research source. The source has 21 declarations / 7
  headlines; 11 axiom rows are empty and 10 depend only on `propext`.
- Stable theorem statuses remain unchanged.
