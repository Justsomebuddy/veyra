...
## Internal inventory — P3-OG machine and first-return pressure — research candidate
The isolated machine facade currently exposes source/seed, transition-kind,
...
An additive, separately namespaced lifecycle facade replays the deterministic
...
drift, role status stays `OPEN`, and promotions stay zero. The raw-cycle lane is
authority-free replay pressure, not historical authentication or primitive
Veyra genealogy. This implementation inventory does not discharge conceptual
`DEF-OG-001..010`: no historical blind/one-shot selection, typed history/full
formation genealogy and first closure, typed post-formation
