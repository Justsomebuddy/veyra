...
| `EndogenousArithmeticObserverRole[D,H,S](o)`; `FirstClosure(M,H,t0,q0)`; `RetainsDifference(M,q0,a,b,k,c)`; `RemovalDependent(M,u,q,C)` | proposed P3-OG observer-role, historical first-closure, retained-residue, and ablation-dependence obligations; all remain `OPEN`; executable evidence is limited to bounded machine pressure and a separate raw-cycle linear first-return analogue that does not discharge `FirstClosure(M,H,t0,q0)` | [doc 168](../168_endogenous_arithmetic_observer_p3og.md) |
