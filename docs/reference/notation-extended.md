...
| `EndogenousArithmeticObserverRole[D,H,S](o)`; `FirstClosure(M,H,t0,q0)`; `RetainsDifference(M,q0,a,b,k,c)`; `RemovalDependent(M,u,q,C)` | proposed P3-OG observer-role, first-closure, retained-residue, and ablation-dependence obligations; all remain `OPEN`, while only a bounded machine-pressure candidate is executable | [doc 168](../168_endogenous_arithmetic_observer_p3og.md) |
